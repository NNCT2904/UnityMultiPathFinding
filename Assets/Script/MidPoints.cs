...
{    
    ...
    public List<MidPoints> midpoints; // refer to list of point to draw path.
    public GameObject parentObject;
    private List<Node> FinalPath = new List<Node>();
    private List<Node> tempPath;
    public GameObject pathPrefab;
    public List<Node> midPointNodes; //refer to the nearest node of the elements in midpoints
    ...
    void Start()
    {
        ...
        //MidPoints[] ts = parentObject.GetComponentsInChildren<MidPoints>();
        //foreach (MidPoints t in ts)
        //{
        //    midpoints.Add(t);
        //}

        //Node thisNode = GridReference.NodeFromWorldPoint(transform.position);

        //FindNearestNode(Thisn)

        //for (int i = 0; i < midpoints.Count; i++)
        //{
        //    Vector3 nodePosition = midpoints[i].GetComponent<Transform>().position;
        //    Debug.Log($"midpoint count: {midpoints.Count}");
        //    Debug.Log($"node position found: x = {nodePosition.x}, y = {nodePosition.y}");
        //    Node temp = GridReference.NodeFromWorldPoint(nodePosition);
        //    midPointNodes.Add(temp);
        //}

        //Debug.Log($"MidpointNodes count: {midPointNodes.Count}");

        //foreach (var node in midPointNodes)
        //{
        //    Debug.Log($"node in MidPointNode position found: x = {node.vPosition.x}, y = {node.vPosition.y}");
        //}
    ...
    void FindNearestNode(Node node, List<Node> midPointsNodes)
    {
        Node less = FindPath(node.vPosition, midpoints[0].transform.position);
        int lessCost = less.FCost;

        for (int i = 0; i < midpoints.Count; i++)
        {
            Node temp = FindPath(node.vPosition, midpoints[i].transform.position);
